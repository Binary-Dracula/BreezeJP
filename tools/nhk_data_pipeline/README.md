...
从 NHK NEWS WEB EASY 获取新闻（带注音）及配音录音，通过 Kuromoji 分词处理，生成供 App 消费的结构化数据。

## 目录结构

```
tools/nhk_data_pipeline/
├── scripts/                    # 所有脚本
│   ├── nhk_scraper.py          # 核心爬虫（抓取+音频下载）
│   ├── align.py                # 音频-文本时间戳对齐（faster-whisper）
│   ├── process_all.js          # Kuromoji 批量分词处理
...
│       ├── {article_id}.mp3    # 音频文件
│       ├── aligned.json        # 音频对齐数据
│       └── processed.json      # Kuromoji 分词及翻译后的最终数据
...
管道自动完成：爬虫 → 音频下载 → Whisper 对齐 → Kuromoji 分词 → ZhipuAI 翻译 → 部署到 App
...
- 每句含 `words` 数组（Kuromoji 分词结果 + ruby_text 注音）
