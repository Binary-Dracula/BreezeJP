...
export async function handleToggleWordFavorite(request, env, auth) {
    let body;
    try {
    ...
    if (existingRows.length > 0) {
        const deleteResp = await supabaseFetch(env, '/user_word_favorites', {
            user_id: `eq.${auth.sub}`,
            word_id: `eq.${wordId}`,
        }, { method: 'DELETE' });
        if (!deleteResp.ok) {
    ...
    const insertResp = await supabaseFetch(env, '/user_word_favorites', undefined, {
        method: 'POST',
        body: {
            user_id: auth.sub,
            word_id: wordId,
...
export async function handleToggleExampleFavorite(request, env, auth) {
    let body;
    try {
        body = await request.json();
    }
    ...
    const wordId = body.word_id?.trim();
    if (!exampleId || !wordId) {
        return errorResponse(400, 'BAD_REQUEST', 'Missing required fields: example_id and word_id');
    }
    ...
    const existingRows = (await existingResp.json());
    if (existingRows.length > 0) {
        const deleteResp = await supabaseFetch(env, '/user_word_example_favorites', {
            user_id: `eq.${auth.sub}`,
            example_id: `eq.${exampleId}`,
        }, { method: 'DELETE' });
        if (!deleteResp.ok) {
    ...
    const insertResp = await supabaseFetch(env, '/user_word_example_favorites', undefined, {
        method: 'POST',
        body: {
            user_id: auth.sub,
...
async function resolveBookIdForWord(env, userId, wordId) {
    ...
    const contentRows = (await contentResp.json());
