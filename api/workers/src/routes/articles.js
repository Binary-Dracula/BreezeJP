 ...
 */
export async function handleArticleList(request, env, _auth) {
    ...
    return jsonResponse({ data: articles, meta }, {
 ...
 */
export async function handleArticleDetail(request, env, _auth, id) {
    // ---- KV 缓存 ----
    ...
    const detailList = (await detailResp.json());
    const items = detailList.length ? detailList[0].items : [];
    const article = { ...meta, items: items };
    // ---- 写入 KV 缓存 ----
