...
export async function handleGrammarDetail(request, env, auth, grammarId) {
...
export async function handleGrammarList(request, env, auth) {
...
export async function handleGrammarStates(request, env, auth) {
    let body;
    try {
    ...
    const response = await supabaseFetch(env, '/user_grammar_states', { on_conflict: 'user_id,grammar_id' }, {
        method: 'POST',
...
async function fetchGrammarDetailBundle(env, userId, grammarId) {
    const [grammarResp, meaningsResp, contextsResp, examplesResp, stateResp] = await Promise.all([
        ...
        supabaseFetch(env, '/user_grammar_states', {
            select: 'grammar_id,learning_status,next_review_at,last_reviewed_at,streak,total_reviews,fail_count,interval,ease_factor,stability,difficulty,created_at,updated_at',
            user_id: `eq.${userId}`,
            grammar_id: `eq.${grammarId}`,
            limit: '1',
        }),
...
function sanitizeJlptLevel(raw) {
    if (raw == null) {
        return null;
    }
