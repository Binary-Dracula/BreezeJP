...
export async function handleHomeSummary(request, env, auth) {
    ...
    const masteredWordCount = parseContentRangeTotal(wordMasteredResp.headers.get('content-range')) ?? 0;
    const kanaMasteredCount = parseContentRangeTotal(kanaMasteredResp.headers.get('content-range')) ?? 0;
    return jsonResponse({
        data: {
            user_name: userName,
