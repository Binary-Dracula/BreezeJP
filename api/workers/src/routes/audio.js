 ...
 * 核心 R2 读取与流式返回逻辑
 */
async function fetchFromR2(env, request, objectKey) {
    const rangeHeader = request.headers.get('Range');
    const object = rangeHeader
        ? await env.AUDIO_BUCKET.get(objectKey, { range: parseRange(rangeHeader) })
        : await env.AUDIO_BUCKET.get(objectKey);
    if (!object) {
        return new Response(JSON.stringify({ error: { code: 'NOT_FOUND', message: `Audio '${objectKey}' not found` } }), { status: 404, headers: { 'Content-Type': 'application/json', ...corsHeaders(request) } });
    }
    const headers = {
 ...
 */
export async function handleAudio(request, env, _auth, id) {
 ...
 */
export async function handleWordAudio(request, env, id) {
