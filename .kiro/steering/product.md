...
目标：让用户在“认读、听辨、形状、笔顺、结构”多维度巩固假名，而不是只做单一的读音回忆。

### MVP 需实现的三种模式

1. **读音回忆（基础 SRS）**：看到假名 → 回忆读音，记录 SRS 复习结果。
2. **听音辨假名**：播放音频 → 从若干假名中选择正确项。
3. **平假名 ↔ 片假名对照**：给定平假名或片假名，要求写出/选择对应的另一套字符。

### 五十音复习题型根据以下规则动态决定：

1. 使用 kana_learning_state 判断假名熟练度（streak、fail_count）
2. 根据熟练度选择题型优先级
3. 使用 kana_logs 避免重复题型
4. 所有题型最终通过 Again / Good / Easy 作为 SRS 输入

结果：
同一个假名每次复习可能出现不同题型，形成更立体的复习体验。

### 后续扩展（版本迭代）

4. **形状测试**：遮挡部分笔画或变体，考察视觉记忆。
5. **笔顺复习**：播放/手写笔顺，强化书写记忆。
6. **行段结构复习**：基于行/段的分组复习，帮助理解假名规律。
