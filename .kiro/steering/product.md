...
Word 的 `seen` 状态**仅允许在以下时机创建**：

* 用户在 Learn 页面中通过左右滑动 **第一次看到该单词**
...
按钮文案始终表示**点击后将发生的状态变化**。

---

### 七、与 Kana 的对齐声明（冻结）

* Word 与 Kana 使用同一套 `LearningStatus`
* Word 的 `seen` ≈ Kana 的“展示即 seen”
* Word 的 `learning` ≈ Kana 的“完成一次学习后进入复习”
* Command / Query / Controller 架构同构
